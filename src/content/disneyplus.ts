...
// Handles video synchronization for Disney+
// Note: Disney+ uses Widevine DRM - we can only control playback via DOM manipulation

import { VideoAdapter } from '../shared/adapter';
import { SyncEvent } from '../shared/events';

class DisneyPlusAdapter implements VideoAdapter {
  private video: HTMLVideoElement | null = null;

  async initialize(): Promise<boolean> {
    try {
      // Disney+ uses standard HTML5 video element with DRM
      this.video = document.querySelector('video');

      if (!this.video) {
        console.warn('[WatchTogether:Disney+] No video element found');
        return false;
      }
      ...
      console.log('[WatchTogether:Disney+] Adapter initialized');
      return true;
    } catch (error) {
      console.error('[WatchTogether:Disney+] Failed to initialize:', error);
      return false;
    }
  }
  ...
  async play(): Promise<void> {
    ...
    try {
      ...
      console.error('[WatchTogether:Disney+] Play failed:', error);
    }
  }
...
class DisneyPlusIntegration {
  private adapter: DisneyPlusAdapter | null = null;
  ...
  private async initialize(): Promise<void> {
    console.log('[WatchTogether:Disney+] Initializing');
  ...
  private async initializeAdapter(): Promise<void> {
    ...
    this.adapter = new DisneyPlusAdapter();
  ...
  private getVideoIdentifier(): string | null {
    try {
      // Disney+ URL structure: /video/[contentId]
      const match = location.pathname.match(/\/video\/([^/]+)/);
      if (match && match[1]) {
        return `disneyplus:${match[1]}`;
      }
      return null;
    } catch {
      return null;
  ...
  private updateVideoIdentifier(): void {
    ...
    if (this.currentVideoId) {
      this.sendToBackground({
        ...
        payload: { url: location.href, platform: 'disneyplus' as const }
      });
    }
  }
  ...
  private observeUrlChanges(): void {
    ...
    new MutationObserver(() => {
      ...
      if (currentUrl !== lastUrl) {
        lastUrl = currentUrl;
        console.log('[WatchTogether:Disney+] URL changed, reinitializing');
        this.adapter = null;
    ...
    switch (message.type) {
      case 'APPLY_SYNC': {
        ...
        if (event.videoId && this.currentVideoId && event.videoId !== this.currentVideoId) {
          console.warn(`[WatchTogether:Disney+] Video mismatch - ignoring sync`);
  ...
  private startPeriodicSync(): void {
    ...
    this.syncCheckInterval = setInterval(() => {
      ...
      console.log('[WatchTogether:Disney+] Periodic sync check');
    }, 3000);
  }
  ...
  private sendToBackground(message: any): void {
    chrome.runtime.sendMessage(message).catch((error) => {
      console.error('[WatchTogether:Disney+] Failed to send message:', error);
    });
  }
}

// Initialize integration
new DisneyPlusIntegration();
