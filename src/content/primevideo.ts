// Amazon Prime Video Content Script
// Handles video synchronization for Prime Video
// Note: Prime Video uses Widevine DRM - we can only control playback via DOM manipulation

import { VideoAdapter } from '../shared/adapter';
import { SyncEvent } from '../shared/events';

class PrimeVideoAdapter implements VideoAdapter {
  private video: HTMLVideoElement | null = null;

  async initialize(): Promise<boolean> {
    try {
      // Prime Video uses standard HTML5 video element with DRM
      this.video = document.querySelector('video');

      if (!this.video) {
        console.warn('[WatchTogether:PrimeVideo] No video element found');
        return false;
      }
      ...
      console.log('[WatchTogether:PrimeVideo] Adapter initialized');
      return true;
    } catch (error) {
      console.error('[WatchTogether:PrimeVideo] Failed to initialize:', error);
      return false;
    }
  }
  ...
  async play(): Promise<void> {
    ...
    try {
      ...
      console.error('[WatchTogether:PrimeVideo] Play failed:', error);
    }
  }
...
class PrimeVideoIntegration {
  private adapter: PrimeVideoAdapter | null = null;
  ...
  private async initialize(): Promise<void> {
    console.log('[WatchTogether:PrimeVideo] Initializing');
  ...
  private async initializeAdapter(): Promise<void> {
    ...
    this.adapter = new PrimeVideoAdapter();
  ...
  private getVideoIdentifier(): string | null {
    try {
      // Prime Video URL structure: /detail/[contentId] or /watch/[contentId]
      const detailMatch = location.pathname.match(/\/detail\/([^/?]+)/);
      if (detailMatch && detailMatch[1]) {
        return `primevideo:${detailMatch[1]}`;
      }

      const watchMatch = location.pathname.match(/\/watch\/([^/?]+)/);
      if (watchMatch && watchMatch[1]) {
        return `primevideo:${watchMatch[1]}`;
      }
  ...
  private updateVideoIdentifier(): void {
    ...
    if (this.currentVideoId) {
      this.sendToBackground({
        ...
        payload: { url: location.href, platform: 'primevideo' as const }
      });
    }
  }
  ...
  private observeUrlChanges(): void {
    ...
    new MutationObserver(() => {
      ...
      if (currentUrl !== lastUrl) {
        lastUrl = currentUrl;
        console.log('[WatchTogether:PrimeVideo] URL changed, reinitializing');
        this.adapter = null;
    ...
    switch (message.type) {
      case 'APPLY_SYNC': {
        ...
        if (event.videoId && this.currentVideoId && event.videoId !== this.currentVideoId) {
          console.warn(`[WatchTogether:PrimeVideo] Video mismatch - ignoring sync`);
  ...
  private startPeriodicSync(): void {
    ...
    this.syncCheckInterval = setInterval(() => {
      ...
      console.log('[WatchTogether:PrimeVideo] Periodic sync check');
    }, 3000);
  }
  ...
  private sendToBackground(message: any): void {
    chrome.runtime.sendMessage(message).catch((error) => {
      console.error('[WatchTogether:PrimeVideo] Failed to send message:', error);
    });
  }
}

// Initialize integration
new PrimeVideoIntegration();
