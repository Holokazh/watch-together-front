// Max (HBO Max) Content Script
// Handles video synchronization for Max (formerly HBO Max)
// Note: Max uses Widevine DRM - we can only control playback via DOM manipulation

import { VideoAdapter } from '../shared/adapter';
import { SyncEvent } from '../shared/events';

class MaxAdapter implements VideoAdapter {
  private video: HTMLVideoElement | null = null;

  async initialize(): Promise<boolean> {
    try {
      // Max uses standard HTML5 video element with DRM
      this.video = document.querySelector('video');

      if (!this.video) {
        console.warn('[WatchTogether:Max] No video element found');
        return false;
      }
      ...
      console.log('[WatchTogether:Max] Adapter initialized');
      return true;
    } catch (error) {
      console.error('[WatchTogether:Max] Failed to initialize:', error);
      return false;
    }
  }
  ...
  async play(): Promise<void> {
    ...
    try {
      ...
      console.error('[WatchTogether:Max] Play failed:', error);
    }
  }
...
class MaxIntegration {
  private adapter: MaxAdapter | null = null;
  ...
  private async initialize(): Promise<void> {
    console.log('[WatchTogether:Max] Initializing');
  ...
  private getVideoIdentifier(): string | null {
    try {
      // Max URL structure: /video/[contentId] or /feature/[contentId]
      const videoMatch = location.pathname.match(/\/video\/([^/?]+)/);
      if (videoMatch && videoMatch[1]) {
        return `max:${videoMatch[1]}`;
      }

      const featureMatch = location.pathname.match(/\/feature\/([^/?]+)/);
      if (featureMatch && featureMatch[1]) {
        return `max:${featureMatch[1]}`;
      }
  ...
  private updateVideoIdentifier(): void {
    ...
    if (this.currentVideoId) {
      this.sendToBackground({
        ...
        payload: { url: location.href, platform: 'max' as const }
      });
    }
  }
  ...
  private observeUrlChanges(): void {
    ...
    new MutationObserver(() => {
      ...
      if (currentUrl !== lastUrl) {
        lastUrl = currentUrl;
        console.log('[WatchTogether:Max] URL changed, reinitializing');
        this.adapter = null;
    ...
    switch (message.type) {
      case 'APPLY_SYNC': {
        ...
        if (event.videoId && this.currentVideoId && event.videoId !== this.currentVideoId) {
          console.warn(`[WatchTogether:Max] Video mismatch - ignoring sync`);
  ...
  private startPeriodicSync(): void {
    ...
    this.syncCheckInterval = setInterval(() => {
      ...
      console.log('[WatchTogether:Max] Periodic sync check');
    }, 3000);
  }
  ...
  private sendToBackground(message: any): void {
    chrome.runtime.sendMessage(message).catch((error) => {
      console.error('[WatchTogether:Max] Failed to send message:', error);
    });
  }
}
